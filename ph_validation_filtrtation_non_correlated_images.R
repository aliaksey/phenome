ph_filter_correllation_per_image("comparison reproducible repeats correlation corr=0.5 feat 345",cell.ftrs.f.scaled,
                               cell.ftrs.reprod, 345)

ph_filter_correllation_per_image("comparison reproducible repeats correlation corr=0.5 feat blank",cell.ftrs.f.scaled,
                                 cell.ftrs.reprod, 2177)
ph_filter_correllation_per_image("comparison reproducible repeats correlation corr=0.5 feat 1682",cell.ftrs.f.scaled,
                                 cell.ftrs.reprod, 1682)
ph_filter_correllation_per_image("comparison reproducible repeats correlation corr=0.5 feat 1029",cell.ftrs.f.scaled,
                                 cell.ftrs.reprod, 1029)
ph_filter_correllation_per_image("comparison reproducible repeats correlation corr=0.5 feat 648",cell.ftrs.f.scaled,
                                 cell.ftrs.reprod, 648)
ph_filter_correllation_per_image("comparison reproducible repeats correlation corr=0.5 feat 200",cell.ftrs.f.scaled,
                                 cell.ftrs.reprod, 200)
ph_filter_correllation_per_image("comparison reproducible repeats correlation corr=0.5 feat 483",cell.ftrs.f.scaled,
                                 cell.ftrs.reprod, 483)
ph_filter_correllation_per_image("comparison reproducible repeats correlation corr=0.5 feat 875",cell.ftrs.f.scaled,
                                 cell.ftrs.reprod, 875)
ph_filter_correllation_per_image("comparison reproducible repeats correlation corr=0.5 feat 2017",cell.ftrs.f.scaled,
                                 cell.ftrs.reprod, 2017)
ph_filter_correllation_per_image("comparison reproducible repeats correlation corr=0.5 feat 76",cell.ftrs.f.scaled,
                                 cell.ftrs.reprod, 76)
ph_filter_correllation_per_image("comparison reproducible repeats correlation corr=0.5 feat 1344",cell.ftrs.f.scaled,
                                 cell.ftrs.reprod, 1344)
ph_filter_correllation_per_image("comparison reproducible repeats correlation corr=0.5 feat 1212",cell.ftrs.f.scaled,
                                 cell.ftrs.reprod, 1212)
ph_filter_correllation_per_image("comparison reproducible repeats correlation corr=0.5 feat 212",cell.ftrs.f.scaled,
                                 cell.ftrs.reprod, 212)
ph_filter_correllation_per_image("comparison reproducible repeats correlation corr=0.5 feat 1173",cell.ftrs.f.scaled,
                                 cell.ftrs.reprod, 1173)
ph_filter_correllation_per_image("comparison reproducible repeats correlation corr=0.5 feat 1918",cell.ftrs.f.scaled,
                                 cell.ftrs.reprod, 1918)
ph_filter_correllation_per_image("comparison reproducible repeats correlation corr=0.5 feat 1407",cell.ftrs.f.scaled,
                                 cell.ftrs.reprod, 1407)
for(i in unique(D$FeatureIdx)){
  
ph_filter_correllation_per_image(paste("show filtration",i,sep=" "),cell.ftrs.f.scaled,
                                 cell.ftrs.reprod,i)
}

ph_filter_correllation_per_image<-function(name, beforefilter, afterfilter, feature)
{
  load("ph_raw_data.RData")
  htmlfilepath<-paste('D:/projects/phenome_project/imageverification/',
                      name,".html", sep = "") 
  ## creating html file
  htmlfile<-file.path(htmlfilepath)
  #specification of html ############################################
  formatspec_head<-'<!DOCTYPE html> \r\n <html> \r\n <body>' 
  formatspec_end<-'</html>' 
  formatspec_h1<-paste('<h1>', name, '</h1>', sep = "")
  formatspec_h1_1<-'<h1> <font color="blue"> ICAm median Intensity </h1>' 
  formatspec_h1_2<-'<h1> Surfaces </h1>' 
  formatspec_h1_3<-'<h1> <font color="blue"> @ </h1>' 
  formatspec_h1_b<-'<h1> <font color="default"> # </h1>' 
  formatspec_h1_n<-'<h1> <font color="red"> 666 </h1>' 
  formatspec_h2<-paste("<h2> Feature number ", name, "</h2>\r\n", sep="")   # specify feature number
  formatspec_links<-'<img src='
  formatspec_h22<-'<h2> Surface number '
  formatspec_h23<-'</h2>\r\n'
  formatspec_filt<-'<h2> Filtered cells </h2>\r\n'
  formatspec_left<-'<h2> Remaining cells </h2>\r\n'
  formatspec_linke<-' width="300" align="middle"></body> \r\n \r\n' 
  formatspec_<-'<hr>' ########
  ## writing data in html file=======
  cat(formatspec_head,file = htmlfile, append=T)
  cat(formatspec_h1,file = htmlfile, append=T)
  cat(formatspec_,file = htmlfile, append=T)
  #=====
  ## write all
  cat(paste(formatspec_h22,feature,formatspec_h23,sep=""),file = htmlfile, append=T)
  ## calculating images that reamins and that was filtered out
  left<-afterfilter[afterfilter$FeatureIdx==feature, c("FeatureIdx", "ImageNumber")]
  filtered<-beforefilter[!beforefilter$ImageNumber%in%afterfilter$ImageNumber&beforefilter$FeatureIdx==feature, 
                         c("FeatureIdx","ImageNumber")]
  ##first plotting filtered data
  cat(formatspec_filt,file = htmlfile, append=T)
  tempp<-image.data[with(image.data, ImageNumber %in% filtered$ImageNumber),]
  for  (ii in unique(tempp$ImageNumber)){
    pth<-paste("../unsupervised_single_cell/", tempp[tempp$ImageNumber==ii,
                                                     "Image_Metadata_FileName_Actin_w_array_name"], sep="") 
    cat(paste(formatspec_links,pth,formatspec_linke,sep=""),file = htmlfile, append=T);
  } 
  ##second plotting remaining  data
  cat(formatspec_left,file = htmlfile, append=T)
  tempp<-image.data[with(image.data, ImageNumber %in% left$ImageNumber),]
  for  (ii in unique(tempp$ImageNumber)){
    
    pth<-paste("../unsupervised_single_cell/", 
               tempp[tempp$ImageNumber==ii,
                     "Image_Metadata_FileName_Actin_w_array_name"], sep="") 
    cat(paste(formatspec_links,pth,formatspec_linke,sep=""),file = htmlfile, append=T);
  } 
  
  cat(formatspec_end,file = htmlfile, append=T)
} 